/* 3.6 Лидерборды (tab root) — два таба. */
...
export function Leaderboard() {
  ...
  const lbQuery = useQuery({
    ...
    queryFn: () => unwrap(api.leaderboard.$get()),
  });
  ...
  return (
    <div className="lu-scr">
      <NavBar plain title="Лидерборды" subtitle={data?.season ?? undefined} />
      <div className="lu-scr__body lu-scr__body--tab">
        ...
        {lbQuery.isPending && <div className="lu-skel" style={{ height: 160, borderRadius: "var(--radius-lg)" }} />}

        {!lbQuery.isPending && rows.length === 0 && (
          <EmptyState
            icon={<I.Trophy />}
            title="Пока пусто"
            description="Сыграйте первые матчи — лидерборд заполнится за сезон."
          />
        )}
